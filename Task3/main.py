...
def get_files_sizes(path):
    ...
    for i in range(len(fileList)):
        ...
        fileList[i] = fileList[i] + "--" + str(fileInfo.st_size) + dicSizeName[str(countSize)]
...
def sortedByAttribute(path):
    ...
    iter = 0
    while (iter < lenMas):
        if bool(re.search("в больницу", glistString[iter][4])):
...
def createJson(sortedCsvFile, path):
    ...
    output = []
    while iter != len(chunks):
        dic = {"№": int(chunks[iter]), "date": chunks[iter + 1], "fio": chunks[iter + 2],
               "allowance": chunks[iter + 3], "description": chunks[iter + 4]}
        output.append(dic)
        iter += 5
...
path = "C:\\Алг\\Lab2\\PythonUniversityLvl2\\Task3"
csvFileFlag = False
